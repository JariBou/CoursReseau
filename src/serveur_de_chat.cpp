...
// 				{
// 					char data[1024];
// 					int dataLength = recv(pollFd.fd, &data[0], 1024, 0);
// 					if (dataLength <= 0)
// 					{
// 						// Déconnexion
...
// 	bool isRunning = true;
//
// 	// Affichage du marqueur de saisie
// 	std::cout << "> ";
// 	while (isRunning)
// 	{
// 		// Saisie de texte
...
//
// 				// Gestion et/ou envoi du message au serveur
// 				if (send(sock, &message[0], message.size(), 0) == SOCKET_ERROR)
// 				{
// 					std::cerr << "failed to connect: " << WSAGetLastError() << std::endl;
// 					return EXIT_FAILURE;
// 				}
...
// 		{
// 			char data[1024];
// 			int dataLength = recv(sock, &data[0], 1024, 0);
// 			if (dataLength <= 0)
// 			{
// 				// On est déconnecté du serveur
// 				if (dataLength == SOCKET_ERROR)
// 					std::cout << "Disconnected from server: error " << WSAGetLastError() << std::endl;
// 				else
...
// 			}
//
// 			std::cout << std::string(data, dataLength) << std::endl;
// 		}
// 	}
//
// 	closesocket(sock);
